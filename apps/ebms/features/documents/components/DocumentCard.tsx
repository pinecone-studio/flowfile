...
// 1. Signature-ийг тусад нь компонент болгов (Complexity: 1)
const SignatureStatus = ({ status }: { status: string }) => {
  ...
  return (
    <div className="flex items-center gap-2">
      <div
        className={`w-2 h-2 rounded-full ${dotClasses[status] || 'bg-gray-500'}`}
      />
...
// 2. Avatar-уудыг тусад нь компонент болгов (Complexity: 1)
const AvatarGroup = ({ docId }: { docId: string }) => (
  <div className="flex -space-x-2 gap-3">
    {[1, 2, 3].map((i) => (
      ...
      >
        <img
          src={`https://i.pravatar.cc/100?u=${docId}${i}`}
          alt="avatar"
...
// 3. Үндсэн Component (Complexity: Одоо бараг 1 болсон)
export const DocumentCard = ({ document }: { document: EmployeeDoc }) => {
  ...
  return (
    <div className="grid grid-cols-[2fr,1.5fr,1fr,1fr,1.5fr,1fr,1.2fr,0.2fr] items-center gap-4 px-6 py-4 hover:bg-white/5 transition-all duration-300 group border-b border-white/5">
      ...
      <div className="text-[13.5px] text-gray-400 group-hover:text-gray-200 transition-colors">
      ...
      <SignatureStatus status={document.status} />

      <AvatarGroup docId={document.id} />

      <div className="text-[13px] text-gray-400 italic">
        {statusLabels[document.status] || 'Canceled'}
      </div>
